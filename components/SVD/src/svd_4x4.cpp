...
void svd_4x4(const float Ain[16], float U[16], float S[16], float V[16])
{
  ...
  matrix_instance_f32 Ain_; mat_init_f32(&Ain_, 4, 4, (float *)Ain);
  float A[4*4]; matrix_instance_f32 A_; mat_init_f32(&A_, 4, 4, (float *)A);
  ...
  matrix_instance_f32 U_; mat_init_f32(&U_, 4, 4, (float *)U);
  matrix_instance_f32 S_; mat_init_f32(&S_, 4, 4, (float *)S);
  matrix_instance_f32 V_; mat_init_f32(&V_, 4, 4, (float *)V);
  mat_trans_f32(&U_, &A_);
  copy_f32(A, U, 16);
  mat_trans_f32(&S_, &A_);
  copy_f32(A, S, 16);
